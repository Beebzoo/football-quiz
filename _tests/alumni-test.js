...
(async () => {
  ...
  run(app, 'S = freshState(["Ale","Bram","Martijn"], false, "alum", 0); newAlum(); render();');
  await tick(60);
  check("Alumni opens on al_play with a full-time whistle",
    ev(app, "S.phase") === "al_play" && stage(app).includes("askEnd()"), ev(app, "S.phase"));
  run(app, 'S = freshState(["Ale","Bram","Martijn"], false, "alum", 10); S.players[0].score = 30; newAlum(); alGiveUp(); alAfter();');
  await tick(60);
